...
ticket_routing = {
    "title": "Ticket routing rules — FHS · FDT · FSE · KABI · HELIOS",
    "source": "Zasady routingu ticketów (FHS/FDT/FSE/KABI/HELIOS) + Helios Support handling instruction",
    "step1": {
        "title": "Step 1 — read the ticket",
        "warning": "Do not route by the sender domain alone. Always establish: which account does the problem concern?",
        "analyse": [
    ...
    "decisionTree": [
        {"no": 1, "question": "Does the problem concern an FHS / FDT / FSE / KABI / Fresenius account?",
         "examples": ["@fresenius.com account", "FHS\\xxxx account", "FDT\\xxxx account",
                      "FSE\\xxxx account", "KABI\\xxxx account"],
         "routing": "FHS / FDT"},
        {"no": 2, "question": "Does the problem concern a HELIOS account?",
         "examples": ["HELIOS-DOM account", "HELIOS mailbox", "HELIOS login",
                      "HELIOS password reset", "HELIOS account lockout"],
         "routing": "HELIOS SUPPORT"},
        {"no": 3, "question": "Does an FHS / FDT / FSE / KABI user need access to a HELIOS system?",
         "examples": ["Access to an application, system or data in the HELIOS environment"],
         "routing": "HELIOS SUPPORT"},
    ],
    "important": {
        "title": "The address the request was sent from does not determine the routing",
        "doNotDecideBy": [
            "Sender address",
    ...
    "examples": [
        {"sent": "Sent from an email account", "concerns": "Problem concerns an FHS / FDT account",
         "result": "FHS / FDT", "not": "NOT Helios"},
        {"sent": "Sent from an email account", "concerns": "Problem concerns a Helios system",
         "result": "HELIOS SUPPORT", "not": "NOT FHS / FDT"},
    ],
    "goldenRule": {
        "do": "Route by the account the problem concerns.",
        "dont": "Do not route by the account the user sent the request from.",
    },
    "commonMistake": {
        "description": "The agent sees the sender domain (e.g. @hermed.de, @fresenius.com, "
                       "@helios-gesundheit.de) and picks the queue straight away.",
        "steps": ["First read the request.",
                  "Check which account the problem concerns.",
                  "Only then set the correct routing."],
    },
    "heliosSupport": {
        "title": "Calls from Helios users — FDT agents",
        "status": "Tickets from users with the Helios domain are not handled by FDT yet.",
        "rules": [
            "Phone: ask the user to contact Helios support.",
            "If someone already redirected the user to us: collect the user information, open a ticket, "
            "assign it to Swivel and hand it over to Aga / Gabi / Filip (they raise it with FDT).",
        ...
        "contacts": [
            {"role": "Helios user contact", "detail": "myservice@helios-gesundheit.de"},
            {"role": "Swivel queue (Email / SSP / Chat)", "detail": "Ext_WW_Swivel-Chair-Helios_Capgemini_Helios"},
            {"role": "Template", "detail": "FDT Misplaced Call Helios related"},
            {"role": "Hand over to", "detail": "Aga / Gabi / Filip"},
        ],
    },
}
